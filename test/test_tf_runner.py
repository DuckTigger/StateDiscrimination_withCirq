...
class TestCirqRunner(np.testing.TestCase):
    ...
    def test_calculate_probabilities(self):
        cirq_runner = CirqRunner(sim_repetitions=100)
        ...
        probs_tf = tf_runner.calculate_probabilities(dicts, tf.constant(zero_state))
        np.testing.assert_array_equal(probs0, [0, 0, 1, 0])
        np.testing.assert_array_equal(probs_tf, probs0)
        ...
        probs_1tf = tf_runner.calculate_probabilities(dicts, tf.constant(one_state))
        np.testing.assert_array_equal(probs1, [0, 1, 0, 0])
        np.testing.assert_array_equal(probs_1tf, probs1)
        ...
        probs2 = cirq_runner.calculate_probabilities_sampling(zero_state, circuit2)
        probs_tf_2 = tf_runner.calculate_probabilities(dicts, tf.constant(zero_state))
