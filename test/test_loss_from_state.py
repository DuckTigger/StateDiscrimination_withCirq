...
class TestLossFromState(tf.test.TestCase):
    ...
    def test_perfect_discrimination(self):
        ...
        oozz[0] = one
        oozz[1] = one
        zero_state = tf.constant(self.kron_list(z_list).astype(np.complex64))
        ozzz_state = tf.constant(self.kron_list(oozz).astype(np.complex64))
        ...
        loss_calc = LossFromState(tf.constant(1., dtype=tf.float64), tf.constant(1., dtype=tf.float64))
        loss_calc.set_all_dicts(gate_dict, gate_dict_0, gate_dict_1)
        states = tf.stack([zero_state, ozzz_state])
    ...
    def test_with_circuit(self):
        ...
        oozz_state = self.kron_list(oozz).astype(np.complex64)
        z_lab = tf.constant(0, dtype=tf.complex64)
        o_lab = tf.constant(1, dtype=tf.complex64)
        labels = tf.stack([z_lab, o_lab])
        loss_calc = LossFromState(tf.constant(1., dtype=tf.float64), tf.constant(1., dtype=tf.float64))
        qubits = loss_calc.runner.get_qubits()
        ...
        controlled_0 = loss_calc.runner.yield_controlled_circuit(circuit_0, qubits[0])
        controlled_1 = loss_calc.runner.yield_controlled_circuit(circuit_1, qubits[0])
        ...
        loss = []
        for state, label in zip(states, labels):
            p = loss_calc.runner.calculate_probabilities(state, circuit)
            probs.append(p)
            l = loss_calc.probs_to_loss(p, label)
            loss.append(l)
        print(probs)
