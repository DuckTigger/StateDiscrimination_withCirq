...
class QSimulator:
    ...
    def partial_trace(self, rho_in, qid):
        ...
        reordered = tf.transpose(reshaped, reordering)
        reshaped2 = tf.reshape(reordered, shape2)

        rho_out = tf.trace(reshaped2)
    ...
    def measure_rho(self, rho_in, qid):
        ...
        prob_1 = tf.trace(tf.matmul(M1, tf.matmul(M1, rho_in), adjoint_a=True))
        prob_1 = tf.reshape(tf.cast(tf.real(prob_1), dtype=tf.float32), [])

        randNum = tf.random_uniform([], 0, 1, dtype=tf.float32)
    ...
    def return_probabilites_0_1(self, rho_in, qid, is_reduced_dm=True):
        ...
        prob_1 = tf.trace(tf.matmul(M1, tf.matmul(M1, rho_in), adjoint_a=True))
        prob_0 = tf.trace(tf.matmul(M0, tf.matmul(M0, rho_in), adjoint_a=True))

        prob_1 = tf.reshape(tf.cast(tf.real(prob_1), dtype=tf.float64), [])
        prob_0 = tf.reshape(tf.cast(tf.real(prob_0), dtype=tf.float64), [])
    ...
    def return_joint_probabilities(self, rho_in, qid_0, qid_1):
        ...
        p00 = tf.trace(tf.matmul(M00, rho_in))
        p01 = tf.trace(tf.matmul(M01, rho_in))
        p10 = tf.trace(tf.matmul(M10, rho_in))
        p11 = tf.trace(tf.matmul(M11, rho_in))

        p00 = tf.reshape(tf.cast(tf.real(p00), dtype=tf.float64), [])
        p01 = tf.reshape(tf.cast(tf.real(p01), dtype=tf.float64), [])
        p10 = tf.reshape(tf.cast(tf.real(p10), dtype=tf.float64), [])
        p11 = tf.reshape(tf.cast(tf.real(p11), dtype=tf.float64), [])
    ...
    def _set_qubit_to_value(self, rho_in, state_to_set: int, qid):
        ...
        measure_one = tf.constant([[0, 0], [0, 1]], dtype=tf.complex128)
        null_state = tf.zeros(tf.shape(rho_in), dtype=tf.complex128)

        if state_to_set:
            M1 = self.gate_matrix_1q(qid, measure_one)
            rho_out = tf.matmul(M1, tf.matmul(rho_in, M1, adjoint_b=True))
            prob_1 = tf.trace(tf.matmul(M1, tf.matmul(M1, rho_in), adjoint_a=True))
            rho_out = tf.cond(tf.equal(prob_1, tf.constant(0, dtype=tf.complex128)), lambda: null_state,
                              lambda: tf.divide(rho_out, prob_1))
        elif state_to_set == 0:
            M0 = self.gate_matrix_1q(qid, measure_zero)
            rho_out = tf.matmul(M0, tf.matmul(rho_in, M0, adjoint_b=True))
            prob_0 = tf.trace(tf.matmul(M0, tf.matmul(M0, rho_in), adjoint_a=True))
            rho_out = tf.cond(tf.equal(prob_0, tf.constant(0, dtype=tf.complex128)), lambda: null_state,
                              lambda: tf.divide(rho_out, prob_0))
        else:
    ...
    def apply_gate_dict(self, gate_dict, rho_in, noise_on=False, noise_prob=0.1):
        ...
        for i in range(len(gate_dict['qid'])):
            if noise_on:
                ...
                norm = tf.trace(rho_out)
                null_state = tf.zeros(tf.shape(rho_in), dtype=tf.complex128)
                rho_out = tf.cond(tf.equal(norm, tf.constant(0, dtype=tf.complex128)), lambda: null_state,
                                  lambda: tf.divide(rho_out, norm))
